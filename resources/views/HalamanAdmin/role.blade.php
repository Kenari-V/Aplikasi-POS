            ...
            <div class="container-product">
                ...
                <div class="box-action-headcontent">
                    <div class="button-tambah-data">
                        ...
                        <a class="create-data-product" href="Role/tambah-users">Tambah</a>
                    </div>
                    <form action="/Role" method="POST" class="form-produk">
                        @csrf
                    ...
                    <form action="/Role" method="GET" id="form-search">
                        <div class="search-data">
                            <a href=""><ion-icon name="search-outline" class="search-data-icon"></ion-icon></a>
                            <input type="search" name="search_users" id="search-input" placeholder="Search Produk" class="search-product-input">
                        </div>
                    </form>
                ...
                <table class="table-border">
                    <thead>
                        ...
                        @if (count($users) > 0)
                            ...
                            @foreach ( $users as $us )
                            <tr>
                                <td><input type="checkbox" name="ids[]" onclick="addDeleteData();" value="{{ $us->id_users }}" class="checkbox-all"></td>
                                <td>{{$i}}</td>
                                <td>{{$us->name}}</td>
                                <td>{{$us->email}}</td>
                                <td>{{$us->level}}</td>
                                <td><a href="/Role/{{$us->id_users}}/edituser"><ion-icon name="create-outline" class="icon-edit-table"></ion-icon></a></td>
                            </tr>
                            @php($i++)
                            @endforeach
                        @else
