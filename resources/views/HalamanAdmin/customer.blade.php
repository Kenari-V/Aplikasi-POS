...
<div class="content-product">
    <div class="table-product">
        <div class="Title-content-product">
            <h2 class="title-product">Daftar Customer</h2>
            <div class="container-product">
                {{-- <form action="/Customer" method="POST" class="form-produk">
                @csrf --}}
                <div class="box-action-headcontent">
                    <div class="button-tambah-data">
                        ...
                        <a class="create-data-product" href="Customer/tambah-customer">Tambah</a>
                    </div>
                    <form action="/Customer" method="POST" class="form-produk">
                        @csrf
                    ...
                    <form action="/Customer" method="GET" id="form-search">
                        <div class="search-data">
                            <a href=""><ion-icon name="search-outline" class="search-data-icon"></ion-icon></a>
                            <input type="search" name="search_customer" id="search-input" placeholder="Search Produk" class="search-product-input">
                        </div>
                    </form>
                ...
                <table class="table-border">
                    <thead>
                        ...
                        @if (count($customer) > 0)
                            ...
                            <tr>
                                <td><input type="checkbox" name="ids[]" onclick="addDeleteData();" value="{{ $cus->id_customer }}" class="checkbox-all"></td>
                                <td>{{$i}}</td>
                                <td>{{$cus->nama_customer}}</td>
                                <td>{{$cus->contact}}</td>
                                <td>{{$cus->address}}</td>
                                <td><a href="/Customer/{{$cus->id_customer}}/editcustomer"><ion-icon name="create-outline" class="icon-edit-table"></ion-icon></a></td>
                            </tr>
                            @php($i++)
                            @endforeach
                        @else
...
<script>
    function addDeleteData(){
        ...
        for (var i = 0 ; i < a.length ; i++){
            if (a[i].checked){
                cust_data += '<input type="hidden" name="customer_data[]" value="'+a[i].value+'">';
            }
        }
